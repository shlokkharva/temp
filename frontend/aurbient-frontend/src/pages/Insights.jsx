          ...
          >
            {cases.map((study, index) => (
              <motion.div
                ...
                className="liquid-glass border border-white/5 rounded-[1.5rem] overflow-hidden flex flex-col justify-between transition-all duration-300 group cursor-pointer h-full"
              >
                {/* Thumbnail Header */}
                <div className="relative h-56 overflow-hidden bg-white/5 w-full">
                  <img
                    src={study.thumbnail}
                    alt={study.client}
                  ...
                  <div className="absolute bottom-4 left-6 flex flex-col gap-1.5">
                    <span className="text-[10px] uppercase tracking-widest font-body text-[#3B82F6] font-semibold bg-black/60 backdrop-blur-md px-3 py-1 rounded-full w-fit">
                      {study.client}
                    </span>
                    <span className="text-xs text-white/70 font-body font-light pl-1">
                      {study.industry}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-8 flex-1 flex flex-col justify-between">
                  <div className="mb-6">
                    <h3 className="font-heading text-white text-3xl tracking-[-1px] mb-4 group-hover:text-[#3B82F6] transition-colors leading-tight">
                      {study.title}
                    </h3>
                  ...
                  {/* Impact Metric Row */}
                  <div className="flex items-center gap-4 mt-6 pt-6 border-t border-white/5 bg-white/[0.01] p-4 rounded-xl">
                    <div className="font-heading text-white text-4xl tracking-[-1px] text-glow-teal text-[#3B82F6]">
                      {study.result_stat}
                    </div>
                    <div className="text-xs text-white/50 font-body font-light leading-tight">
                      {study.result_label}
                    </div>
                  </div>
                </div>
                ...
                {/* Footer Publish Date */}
                <div className="px-8 pb-6 text-[10px] font-body text-white/30 text-right uppercase tracking-wider">
