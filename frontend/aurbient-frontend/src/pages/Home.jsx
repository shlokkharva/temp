...
export default function Home() {
  return (
    <div style={{ background: "#111111", color: "#fff", fontFamily: "'Barlow',sans-serif" }}>
      ...
      {/* ══════════════════════════════════════════ CAPABILITIES */}
      <section style={{ padding: "7rem 5rem", background: "#1a1a1a" }}>
        <motion.div initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.8}}>
          ...
          <h2 style={{ fontWeight: 600, fontSize:"clamp(2.4rem,4vw,3.8rem)", lineHeight:1.0, letterSpacing:"-1.5px", margin:"0.6rem 0 3rem", color:"#fff" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:"1.25rem" }}>
          {[
            ...
            <motion.div
              ...
              whileHover={{ scale:1.02, boxShadow:`0 0 35px rgba(59,130,246,0.12)` }}
              style={{ background:"rgba(255,255,255,0.02)", border:`1px solid ${BORD}`, borderRadius:"1.25rem", padding:"2rem", display:"flex", flexDirection:"column", gap:"1rem", cursor:"pointer", transition:"border-color 0.3s" }}
            ...
            >
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                ...
                <div style={{ display:"flex", gap:"0.35rem", flexWrap:"wrap", justifyContent:"flex-end" }}>
                  {c.tags.map(t => <span key={t} style={{ fontSize:"0.57rem", padding:"2px 8px", borderRadius:"999px", background:"rgba(59,130,246,0.1)", color:"rgba(255,255,255,0.4)", letterSpacing:"0.05em" }}>{t}</span>)}
                </div>
              </div>
              <h3 style={{ fontWeight: 600, fontSize:"1.55rem", margin:0, color:"#fff", letterSpacing:"-0.5px" }}>{c.title}</h3>
              <p style={{ fontSize:"0.78rem", color:DIM, lineHeight:1.75, margin:0, fontWeight:300 }}>{c.body}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding:"7rem 5rem", background: BG }}>
        ...
        {[["150+","Projects Delivered Across Global Technology Sectors"],["98%","Client Retention — Relationships Built to Last"],["40%","Average Operational Efficiency Improvement"],["12","Countries Where Our Systems Run"]].map(([val, label]) => (
          <motion.div
            ...
            style={{ display:"flex", alignItems:"baseline", justifyContent:"space-between", borderBottom:`1px solid ${BORD}`, padding:"2rem 0", gap:"2rem" }}
          >
            <span style={{ fontWeight: 600, fontSize:"clamp(3.5rem,7vw,6.5rem)", lineHeight:1, color:"#fff", letterSpacing:"-2px" }}>
              <AnimatedCounter value={val} />
            </span>
            <span style={{ fontSize:"0.88rem", color:DIM, textAlign:"right", maxWidth:"350px", lineHeight:1.55, fontWeight:300 }}>{label}</span>
          </motion.div>
        ))}
      </section>

      {/* ══════════════════════════════════════════ CTA */}
      <section style={{ padding:"7rem 5rem", background:"#1a1a1a", display:"flex", flexDirection:"column", alignItems:"center", gap:"1.6rem", backgroundImage:`radial-gradient(ellipse 60% 50% at 50% 50%, rgba(59,130,246,0.1) 0%, transparent 70%)` }}>
        <span style={{ color:TEAL, fontSize:"0.67rem", fontWeight:600, letterSpacing:"0.22em", textTransform:"uppercase", background:"rgba(59,130,246,0.06)", padding:"5px 18px", borderRadius:"999px", border:"1px solid rgba(59,130,246,0.18)" }}>
          Ready when you are
        </span>
        <h2 style={{ fontWeight: 600, fontSize:"clamp(2.5rem,4.5vw,4rem)", color:"#fff", margin:0, letterSpacing:"-1.5px", lineHeight:1.05, textAlign:"center" }}>
          Build the future<br />of your business
        </h2>
        <p style={{ fontSize:"0.83rem", color:DIM, maxWidth:"380px", lineHeight:1.75, fontWeight:300, margin:0, textAlign:"center" }}>
          Automate workflow bottlenecks and scale your operational edge with neural precision.
        </p>
        <div style={{ display:"flex", gap:"1rem", flexWrap:"wrap", justifyContent:"center" }}>
          <Link to="/contact" style={{ padding:"12px 30px", borderRadius:"999px", background:`linear-gradient(135deg,rgba(59,130,246,0.22),rgba(59,130,246,0.1))`, border:"1px solid rgba(59,130,246,0.3)", color:"#fff", textDecoration:"none", fontSize:"0.83rem", fontWeight:500 }}>
            Start Your Journey ↗
          </Link>
          <Link to="/insights" style={{ padding:"12px 30px", borderRadius:"999px", background:"transparent", border:`1px solid ${BORD}`, color:DIM, textDecoration:"none", fontSize:"0.83rem", fontWeight:400 }}>
            View Case Studies
          </Link>
        </div>
      </section>
